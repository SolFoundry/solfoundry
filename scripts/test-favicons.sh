...
#
# Runs 25+ assertions that verify every acceptance criterion from issue #471:
...
assert_no_grep "No .ico reference in HTML (PNG-only strategy)" 'favicon\.ico' "$INDEX"
assert_grep "MIME type image/png on link tags" 'image/png'     "$INDEX"
...
assert_no_grep "No type mismatch (image/png → .ico)" 'image/png.*favicon\.ico' "$INDEX"
