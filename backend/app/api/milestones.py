...
async def create_milestones(
    ...
    """
    caller_id = user.wallet_address or str(user.id)
    try:
...
async def list_milestones(
    ...
    """
    try:
        return await milestone_service.get_milestones(db=db, bounty_id=bounty_id)
    except BountyNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
...
class MilestoneRejectRequest(MilestoneSubmitRequest):
    """Request body for rejecting a milestone with an optional reason.

    Args (fields):
        evidence: The rejection reason explaining what needs to change.
    """
