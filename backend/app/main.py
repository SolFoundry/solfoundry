...
from app.services.event_indexer_service import (
    periodic_health_check as periodic_indexer_health_check,
    start_queue_worker,
    stop_queue_worker,
)
...
async def lifespan(app: FastAPI):
    ...
    # Start event indexer queue worker and health monitor
    indexer_queue_task = start_queue_worker()
    indexer_health_task = asyncio.create_task(periodic_indexer_health_check(interval_seconds=60))
    ...
    try:
        ...
        pass
    stop_queue_worker()
    indexer_health_task.cancel()
    try:
        await indexer_queue_task
    except asyncio.CancelledError:
        pass
    try:
        await indexer_health_task
    except asyncio.CancelledError:
        pass
...
# Event Indexer: /api/webhooks/helius, /api/webhooks/shyft, /api/indexed-events/*
