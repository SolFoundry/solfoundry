...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    Sanitizes the error message to avoid leaking internal library
    details (e.g. Pydantic internals) when the message is unexpectedly
    long or contains stack traces.
    """
    request_id = getattr(request.state, "request_id", None)
    raw_message = str(exc)
    # Truncate overly long messages to prevent information leakage
    message = raw_message if len(raw_message) < 200 else "Validation error"
    return JSONResponse(
        status_code=400,
        content={
            "message": message,
...
# Milestones: /api/bounties/{bounty_id}/milestones, /api/milestones/{id}/*
