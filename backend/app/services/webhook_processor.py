...
class WebhookProcessor:
    ...
    async def _update_bounty_status(
        ...
        if not bounty:
            logger.info(
                "Bounty not found for issue #%d in %s",
    ...
    async def _create_bounty_from_issue(
        ...
        if existing:
            logger.info(
                "Bounty already exists for issue #%d",
                github_issue_number
            )
            return None
        ...
        category = "other"
        for label in labels:
            if label in ["frontend", "backend", "smart_contract", "documentation", "testing", "infrastructure"]:
                category = label
                break
