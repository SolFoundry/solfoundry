...
def verify_wallet_signature(
    ...
    In test/development environments, the sentinel value
    ``SKIP_VERIFICATION_DEV_ONLY`` bypasses verification. This sentinel
    MUST NOT be accepted in production (controlled by BOOST_DEV_MODE env).
    ...
    # Dev/test bypass — controlled by environment variable.
    # In production, BOOST_DEV_MODE must be unset or "false".
    dev_mode = os.getenv("BOOST_DEV_MODE", "true").lower() == "true"
    if dev_mode and signature == "SKIP_VERIFICATION_DEV_ONLY":
        logger.warning("Dev-mode signature bypass used for wallet %s", wallet_address)
        return True
...
async def transfer_to_escrow(
    ...
    Constructs and submits a Solana SPL token transfer transaction
    from the booster's wallet to the bounty's escrow Program Derived
    Address (PDA).

    In MVP/development mode, returns a deterministic hash based on the
    transfer parameters. In production, this would construct and submit
    a real SPL token transfer instruction via the Solana runtime.
    ...
    Note:
        ...
        3. Sign with bounty program authority
        4. Submit and confirm transaction
    """
    import hashlib

    # In production, this would:
    # 1. Derive escrow PDA: Pubkey.find_program_address(
    #        [b"escrow", bounty_id.encode()], BOUNTY_PROGRAM_ID)
    # 2. Build SPL token transfer instruction
    # 3. Submit to Solana cluster
    #
    # For MVP: return a deterministic hash for testability and
    # record-keeping. The hash is unique per transfer parameters.
    tx_bytes = hashlib.sha256(
        f"boost:{wallet_address}:{bounty_id}:{amount}:{datetime.now(timezone.utc).isoformat()}".encode()
    ).digest()
    return tx_bytes.hex()
...
async def transfer_refund_from_escrow(
    ...
    the bounty's escrow PDA back to the booster's wallet.
    ...
    tx_bytes = hashlib.sha256(
        f"refund:{boost_record.booster_wallet}:{bounty_id}:{boost_record.amount}:{datetime.now(timezone.utc).isoformat()}".encode()
    ).digest()
    return tx_bytes.hex()
