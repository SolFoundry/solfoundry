...
async def link_wallet_to_user(
    ...
    Verifies the wallet signature, checks for conflicts with other users,
    and updates the user record with the verified wallet address.

    Args:
        db: Async database session.
        user_id: The UUID of the user linking the wallet.
        wallet: The Solana wallet address to link.
        signature: The base64-encoded signature proving wallet ownership.
        message: The challenge message that was signed.
        nonce: Optional nonce for challenge validation.

    Returns:
        Dictionary with success status, message, and updated user profile.

    Raises:
        WalletVerificationError: If the signature is invalid.
        AuthError: If the wallet is already linked to another user or user not found.
    """
...
async def refresh_access_token(db: AsyncSession, refresh_token: str) -> Dict:
    ...
    Decodes the refresh token, validates it, and issues a new
    access token if the user still exists.
    ...
    Raises:
        InvalidTokenError: If the token is malformed or the user is gone.
...
async def get_current_user(db: AsyncSession, user_id: str) -> UserResponse:
    ...
    Converts the string user_id to a UUID object for cross-database
    compatibility (PostgreSQL native UUID vs SQLite CHAR(36)).
    ...
    Returns:
        UserResponse with the user's profile.
