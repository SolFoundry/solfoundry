...
# ---------------------------------------------------------------------------
# Event indexer exceptions
# ---------------------------------------------------------------------------

class EventIndexerError(Exception):
    """Base class for all event indexer errors."""


class DuplicateEventError(EventIndexerError):
    """Raised when a duplicate event (same tx signature + log index) is detected."""


class EventNotFoundError(EventIndexerError):
    """Raised when an indexed event is not found by its signature."""


class WebhookSignatureError(EventIndexerError):
    """Raised when webhook HMAC signature verification fails."""


class BackfillError(EventIndexerError):
    """Raised when a historical backfill operation fails."""
