...
def upgrade() -> None:
    """Create the milestones table with indexes, constraints, and CHECK guards."""
    op.create_table(
        "milestones",
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
        # Database-level CHECK constraints for defense in depth
        sa.CheckConstraint("milestone_number >= 1", name="ck_milestones_number_positive"),
        sa.CheckConstraint("percentage > 0 AND percentage <= 100", name="ck_milestones_percentage_range"),
    )

    # Composite unique index on (bounty_id, milestone_number) — also
    # serves lookups filtered by bounty_id since it is the leading column,
    # so no separate single-column bounty_id index is needed.
    op.create_index(
