...
export function SkeletonCard({
  ...
  return (
    <div
      className={`rounded-xl border border-gray-200 dark:border-surface-300 bg-white dark:bg-surface-50 p-4 sm:p-5 ${className}`}
      role="presentation"
      aria-hidden="true"
      {...props}
    >
  ...
  return (
    <div className={`space-y-4 ${className}`} role="status" aria-label="Loading bounties">
      {Array.from({ length: count }, (_, i) => (
        <div
          key={i}
          className="rounded-xl border border-gray-200 dark:border-surface-300 bg-white dark:bg-surface-50 p-4 hover:border-solana-purple/30 transition-colors"
        >
