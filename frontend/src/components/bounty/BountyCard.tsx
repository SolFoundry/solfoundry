...
export function BountyCard({ bounty }: BountyCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Row 4: Reward + Meta */}
      <div className="flex flex-col gap-3 mt-3 min-[460px]:flex-row min-[460px]:items-center min-[460px]:justify-between">
        <span className="font-mono text-lg font-semibold text-emerald">
          {formatCurrency(bounty.reward_amount, bounty.reward_token)}
        </span>
        <div className="flex items-center gap-x-3 gap-y-2 text-xs text-text-muted flex-wrap">
      ...
      {/* Status badge */}
      <span className={`mt-4 text-xs font-medium inline-flex items-center gap-1 ${statusColor}`}>
