...
/**
 * BountyDetailPage — Full bounty detail view with escrow integration.
 * Displays bounty metadata, submissions, review scores, lifecycle timeline,
 * and the escrow panel for on-chain funding operations.
 *
 * Uses the wallet adapter as the single source of truth for connected wallet identity
 * (not localStorage) to determine ownership and available actions.
 *
 * @module components/BountyDetailPage
 */
...
export const BountyDetailPage: React.FC<{ bounty: BountyDetail }> = ({ bounty }) => {
  ...
  /** Use wallet adapter as the single source of truth for connected wallet identity. */
  const { publicKey: connectedWalletKey } = useWallet();
  const currentUserWallet = connectedWalletKey?.toBase58() ?? '';
  const isCreator = Boolean(bounty.created_by && bounty.created_by === currentUserWallet);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {/* Escrow Integration Panel */}
            <EscrowPanel
              bountyId={bounty.id}
              rewardAmount={rewardAmount}
              bountyStatus={bounty.status}
              ownerWallet={bounty.created_by}
              contributorWallet={bounty.winner_wallet}
            />
          </div>
        </div>
      </div>
