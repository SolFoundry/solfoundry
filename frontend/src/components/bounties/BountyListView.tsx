...
function BountyRow({
  ...
  const urg =
    b.status === "open" &&
    !exp &&
    new Date(b.deadline).getTime() - Date.now() < 2 * 864e5;

  const row = (
    <div
      className={
        "flex items-center gap-4 px-4 py-3 rounded-lg border border-gray-200 dark:border-surface-300 bg-white dark:bg-surface-50 hover:border-solana-green/40 transition-all" +
        (exp ? " opacity-60" : "")
      }
    >
        ...
        <div className="flex items-center gap-3 mt-1">
          ...
          <div className="flex flex-wrap gap-1">
            {b.skills.slice(0, 3).map((s) => (
              <span
                key={s}
                className="rounded-full bg-gray-100 dark:bg-surface-200 px-1.5 py-0.5 text-[10px] text-gray-400"
              >
                {s}
              </span>
            ))}
      ...
      <div className="flex items-center gap-6 shrink-0 text-right">
        ...
        <div className="w-20 text-center">
          <span
            className={"text-xs " + (urg ? "text-[#FF6B6B]" : "text-gray-500")}
          >
  ...
  return (
    <button
      ...
      className="block w-full text-left focus-visible:ring-2 focus-visible:ring-solana-green rounded-lg"
    >
      {row}
    </button>
  );
...
export function BountyListView({
  ...
  return (
    <div className="flex flex-col gap-2" data-testid="bounty-list">
      {bounties.map((b) => (
        <BountyRow key={b.id} bounty={b} onClick={onBountyClick} />
      ))}
    </div>
  );
}
