...
const MOCK_BOUNTIES: Bounty[] = [
  { id: '1', title: 'GitHub 鈫?Platform Bi-directional Sync', reward: 450000, deadline: '2026-03-27', status: 'in_progress', progress: 60 },
...
const MOCK_ACTIVITIES: Activity[] = [
  ...
  { id: '4', type: 'bounty_claimed', title: 'Bounty Claimed', description: 'Claimed "GitHub 鈫?Platform Sync"', timestamp: '2026-03-19T12:00:00Z' },
...
function QuickActions({ onBrowseBounties, onViewLeaderboard, onCheckTreasury }: QuickActionsProps) {
  const actions = [
    { label: 'Browse Bounties', icon: '馃攳', onClick: onBrowseBounties, color: 'from-[#9945FF] to-[#9945FF]' },
    { label: 'View Leaderboard', icon: '馃弳', onClick: onViewLeaderboard, color: 'from-[#14F195] to-[#14F195]' },
    { label: 'Check Treasury', icon: '馃挵', onClick: onCheckTreasury, color: 'from-yellow-500 to-yellow-500' },
  ];
          ...
          {linkedAccounts.map((account) => (
            <div key={account.type} className="flex items-center justify-between py-2 px-3 bg-[#0a0a0a] rounded-lg">
              <div className="flex items-center gap-3">
                <span className="text-lg">{account.type === 'github' ? '馃悪' : account.type === 'twitter' ? '馃惁' : '馃攼'}</span>
                <div>
