 ...
 * based on the escrow state, bounty status, and connected wallet identity
 * from the wallet adapter (not localStorage).
 *
 ...
 * - The connected wallet from the wallet adapter (only the owner can act)
 *
...
export function EscrowPanel({
  ...
  const handleDeposit = useCallback(async () => {
    ...
    try {
      ...
      /* Error is tracked in transactionProgress state. */
    }
  ...
  return (
    <div className="space-y-4" data-testid="escrow-panel">
        ...
        <div className="bg-gray-900 rounded-lg p-4 sm:p-6 space-y-3">
          ...
          {canDeposit && (
            <button
              ...
              className="w-full py-3 rounded-lg bg-gradient-to-r from-purple-600 to-green-500 text-white font-bold hover:from-purple-500 hover:to-green-400 transition-all min-h-[44px] touch-manipulation active:scale-[0.98]"
          ...
          {canRelease && (
            <button
              ...
              className="w-full py-3 rounded-lg bg-green-600 text-white font-bold hover:bg-green-500 transition-colors min-h-[44px] touch-manipulation active:scale-[0.98]"
          ...
          {canRefund && (
            <button
              ...
              className="w-full py-3 rounded-lg bg-yellow-600 text-white font-bold hover:bg-yellow-500 transition-colors min-h-[44px] touch-manipulation active:scale-[0.98]"
      ...
      {/* Transaction confirmation overlay — shows during any active transaction */}
      {transactionProgress.step !== 'idle' && (
        <TransactionConfirmation
          ...
          onRetry={() => {
            resetTransaction();
            /** Re-open the modal for the failed operation type to allow retry. */
