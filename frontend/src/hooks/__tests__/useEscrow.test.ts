/**
 * Tests for the useEscrow hook's query key factory and exported constants.
 * Validates escrow query key generation for cache consistency across
 * account queries and transaction history queries.
 *
...
describe('escrowKeys', () => {
  ...
  it('transactions key includes the base key as prefix', () => {
    const key = escrowKeys.transactions('test-id');
    expect(key[0]).toBe('escrow');
    expect(key[1]).toBe('transactions');
    expect(key[2]).toBe('test-id');
  });

  it('account and transactions keys are distinct for same bounty', () => {
    const accountKey = escrowKeys.account('bounty-1');
    const txKey = escrowKeys.transactions('bounty-1');
    expect(accountKey).not.toEqual(txKey);
    expect(accountKey[1]).toBe('account');
    expect(txKey[1]).toBe('transactions');
  });
});
